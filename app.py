...
def upload_template_to_drive(file_name, file_data):
    try:
        ...
        if results:
            ...
            # 2. 파일이 없으면 'base_template.xlsx'를 찾아서 복사 시도
            base_query = f"name = 'base_template.xlsx' and '{FOLDER_ID}' in parents"
            base_file = service.files().list(q=base_query).execute().get('files', [])
            
            if base_file:
                # 원본 파일을 복사해서 새로운 이름 부여
                copied_file = service.files().copy(
                    fileId=base_file[0]['id'],
                    body={'name': file_name}
                ).execute()
                # 복사된 파일에 데이터 덮어쓰기
                service.files().update(fileId=copied_file['id'], media_body=media).execute()
                return True
            else:
                # base 파일도 없다면 어쩔 수 없이 생성을 시도하지만, 에러 날 확률 높음
